...
# tag: "" = none, "beta" = BETA badge, "new" = NEW badge
INDICATORS = {
    ...
    # ── Beta indicators ───────────────────────────────────────────────────────
    "acc_dist":      ("Accumulation/Distribution", lambda: """//@version=6\nindicator("Accumulation/Distribution", overlay=false)\nfloat clv = ((close - low) - (high - close)) / (high - low)\nfloat ad = ta.cum(na(clv) ? 0 : clv * volume)\nplot(ad, "A/D", color=color.new(color.teal, 0), linewidth=2)\nhline(0, color=color.new(color.gray, 60))\n""", "Cumulative money flow indicator. Rising A/D with rising price confirms the trend. Divergence signals potential reversal.", "volume", "Add as a separate panel. When A/D rises alongside price, buying pressure is genuine. When price rises but A/D falls, distribution is occurring — a bearish warning. Works on any timeframe.", "beta"),
    "adl":           ("Advance Decline Line",      lambda: """//@version=6\nindicator("Advance Decline Line", overlay=false)\nfloat change = close - close[1]\nfloat adl = ta.cum(change > 0 ? 1 : change < 0 ? -1 : 0)\nplot(adl, "ADL", color=color.new(color.blue, 0), linewidth=2)\nplot(ta.ema(adl, 20), "Signal", color=color.new(color.orange, 0), linewidth=1)\n""", "Cumulative breadth indicator tracking advancing vs declining bars. Divergence from price warns of weak trends.", "momentum", "Add as a separate panel. ADL rising = more bars closing up than down (healthy trend). ADL falling while price rises = hidden weakness. Orange line is a 20-bar EMA signal.", "beta"),
    "adr_ratio":     ("Advance Decline Ratio",     lambda: """//@version=6\nindicator("Advance Decline Ratio", overlay=false)\nfloat up   = close > close[1] ? 1.0 : 0.0\nfloat down = close < close[1] ? 1.0 : 0.0\nfloat adr  = ta.sma(up, 20) / math.max(ta.sma(down, 20), 0.001)\nplot(adr, "ADR", color=adr > 1 ? color.new(color.green, 0) : color.new(color.red, 0), linewidth=2)\nhline(1, "Neutral", color=color.new(color.gray, 40), linestyle=hline.style_dashed)\n""", "Ratio of advancing to declining bars over a 20-bar window. Above 1 = more ups than downs.", "momentum", "Add as a separate panel. Above 1.0 = bullish breadth, below 1.0 = bearish. Spikes above 2 often mark short-term tops. Drops below 0.5 can signal washout bottoms.", "beta"),
    "adr_bars":      ("Advance/Decline Ratio (Bars)", lambda: """//@version=6\nindicator("Advance/Decline Ratio Bars", overlay=false)\nfloat up   = close > close[1] ? 1.0 : 0.0\nfloat down = close < close[1] ? 1.0 : 0.0\nfloat ratio = ta.sma(up, 10) / math.max(ta.sma(down, 10), 0.001)\nbool bull = ratio >= 1\ncolor bc = bull ? color.new(color.green, 20) : color.new(color.red, 20)\nplot(ratio, "Ratio", style=plot.style_histogram, color=bc)\nhline(1, color=color.new(color.gray, 40), linestyle=hline.style_dashed)\n""", "Bar-chart version of the A/D ratio — green bars above 1 = bullish, red bars below = bearish.", "momentum", "Add as a separate panel. Green histogram bars above the 1.0 line = more advancing than declining bars. Red bars below = selling pressure. Useful for spotting momentum shifts.", "beta"),
    "alma":          ("Arnaud Legoux MA",           lambda: """//@version=6\nindicator("Arnaud Legoux Moving Average", overlay=true)\nint   len    = input.int(20, "Length", minval=1)\nfloat offset = input.float(0.85, "Offset", minval=0, maxval=1, step=0.05)\nfloat sigma  = input.float(6.0,  "Sigma",  minval=0.1, step=0.5)\nfloat alma   = ta.alma(close, len, offset, sigma)\ncolor ac = close > alma ? color.new(color.green, 0) : color.new(color.red, 0)\nplot(alma, "ALMA", color=ac, linewidth=2)\n""", "Gaussian-weighted moving average with reduced lag. Smoother than EMA with less whipsaw.", "trend", "Paste onto your price chart. Green = price above ALMA (bullish), red = price below (bearish). Adjust Offset (0–1) to shift the weight toward recent bars; higher Sigma = smoother line.", "beta"),
    "aroon":         ("Aroon",                      lambda: """//@version=6\nindicator("Aroon", overlay=false)\nint len = input.int(14, "Length", minval=1)\n[aroonUp, aroonDown] = ta.aroon(high, low, len)\nplot(aroonUp,   "Aroon Up",   color=color.new(color.green, 0), linewidth=2)\nplot(aroonDown, "Aroon Down", color=color.new(color.red,   0), linewidth=2)\nhline(70, "Strong", color=color.new(color.green, 60), linestyle=hline.style_dashed)\nhline(30, "Weak",   color=color.new(color.red,   60), linestyle=hline.style_dashed)\nbgcolor(aroonUp > 70 and aroonDown < 30 ? color.new(color.green, 92) : aroonDown > 70 and aroonUp < 30 ? color.new(color.red, 92) : na)\n""", "Measures how recently the highest high and lowest low occurred. Identifies trend strength and direction.", "trend", "Add as a separate panel. Aroon Up above 70 + Aroon Down below 30 = strong uptrend (green background). Opposite = strong downtrend. Crossing lines signal a trend change.", "beta"),
    "aroon_osc":     ("Aroon Oscillator",           lambda: """//@version=6\nindicator("Aroon Oscillator", overlay=false)\nint len = input.int(14, "Length", minval=1)\n[aroonUp, aroonDown] = ta.aroon(high, low, len)\nfloat osc = aroonUp - aroonDown\ncolor oc = osc >= 0 ? color.new(color.green, 20) : color.new(color.red, 20)\nplot(osc, "Aroon Osc", style=plot.style_histogram, color=oc)\nplot(osc, "Line", color=color.new(oc, 0), linewidth=1)\nhline(0,   color=color.new(color.gray, 40))\nhline(50,  color=color.new(color.green, 60), linestyle=hline.style_dashed)\nhline(-50, color=color.new(color.red,   60), linestyle=hline.style_dashed)\n""", "Aroon Up minus Aroon Down in a single oscillator. Above 0 = bullish trend, below 0 = bearish.", "trend", "Add as a separate panel. Values above 0 indicate an uptrend; below 0 a downtrend. Above +50 = strong bull momentum. Below -50 = strong bear momentum. Zero-line crossovers are trend-change signals.", "beta"),
    "auto_fib_ext":  ("Auto Fib Extension",         lambda: """//@version=6\nindicator("Auto Fib Extension", overlay=true, max_bars_back=500)\nint lb = input.int(20, "Lookback", minval=5)\nfloat lo = ta.lowest(low,   lb)\nfloat hi = ta.highest(high, lb)\nfloat rng = hi - lo\nlevels = array.from(0.0, 0.236, 0.382, 0.5, 0.618, 0.786, 1.0, 1.272, 1.618, 2.0, 2.618)\nlabels = array.from("0", "23.6", "38.2", "50", "61.8", "78.6", "100", "127.2", "161.8", "200", "261.8")\nfor i = 0 to array.size(levels) - 1\n    float lvl = hi + rng * array.get(levels, i)\n    line.new(bar_index - lb, lvl, bar_index, lvl, color=color.new(color.blue, 60), style=line.style_dashed, extend=extend.right)\n    label.new(bar_index, lvl, array.get(labels, i) + "%", style=label.style_label_left, size=size.tiny, color=color.new(color.blue, 80), textcolor=color.white)\n""", "Automatically plots Fibonacci extension levels from the most recent swing. Projects profit targets above the trend.", "trend", "Paste onto your price chart. Extension levels project above the recent swing high. Use 127.2%, 161.8%, and 261.8% as price targets after a breakout. Adjust Lookback to capture the correct swing.", "beta"),
    "auto_fib_ret":  ("Auto Fib Retracement",       lambda: """//@version=6\nindicator("Auto Fib Retracement", overlay=true, max_bars_back=500)\nint lb = input.int(20, "Lookback", minval=5)\nfloat lo = ta.lowest(low,   lb)\nfloat hi = ta.highest(high, lb)\nfloat rng = hi - lo\nlevels = array.from(0.0, 0.236, 0.382, 0.5, 0.618, 0.786, 1.0)\nlabels = array.from("0%", "23.6%", "38.2%", "50%", "61.8%", "78.6%", "100%")\nfor i = 0 to array.size(levels) - 1\n    float lvl = hi - rng * array.get(levels, i)\n    color lc = array.get(levels, i) == 0.618 ? color.new(color.gold, 30) : color.new(color.blue, 60)\n    line.new(bar_index - lb, lvl, bar_index, lvl, color=lc, style=line.style_dashed, extend=extend.right)\n    label.new(bar_index, lvl, array.get(labels, i), style=label.style_label_left, size=size.tiny, color=color.new(lc, 70), textcolor=color.white)\n""", "Automatically draws Fibonacci retracement levels from the most recent swing high to low.", "trend", "Paste onto your price chart. Gold line = 61.8% (the 'golden ratio' — strongest support). Blue dashed lines show other key levels. Price often bounces from 38.2%, 50%, or 61.8%. Adjust Lookback to match your swing.", "beta"),
    "auto_pitchfork":("Auto Pitchfork",             lambda: """//@version=6\nindicator("Auto Pitchfork", overlay=true, max_bars_back=500)\nint lb = input.int(30, "Lookback", minval=10)\nfloat p1y = ta.highest(high, lb)\nint   p1x = bar_index - lb + ta.highestbars(high, lb)\nfloat p2y = ta.lowest(low,  lb / 2)\nint   p2x = bar_index - lb/2 + ta.lowestbars(low, lb/2)\nfloat p3y = ta.lowest(low,  lb)\nint   p3x = bar_index - lb + ta.lowestbars(low, lb)\nfloat mid  = (p2y + p3y) / 2\nfloat slope = (mid - p1y) / math.max(bar_index - p1x, 1)\nfor i = 0 to 10\n    int bx = bar_index + i * 5\n    float cy = p1y + slope * (bx - p1x)\n    float uy = cy + (p2y - mid)\n    float ly = cy - (p2y - mid)\n    line.new(bx, cy, bx + 1, cy + slope, color=color.new(color.orange, 40), width=1)\n""", "Automatically fits an Andrews Pitchfork to the three most recent pivot points.", "trend", "Paste onto your price chart. The median line (center) acts as a magnet — price tends to return to it. Upper fork = resistance, lower fork = support. Works best on daily and 4h charts with clear swings.", "beta"),
    "auto_trendlines":("Auto Trendlines",           lambda: """//@version=6\nindicator("Auto Trendlines", overlay=true, max_bars_back=500)\nint lb = input.int(50, "Lookback", minval=20)\nbool ph = ta.pivothigh(high, 5, 5)\nbool pl = ta.pivotlow(low,  5, 5)\nvar int   r_x1 = na\nvar float r_y1 = na\nvar int   s_x1 = na\nvar float s_y1 = na\nif ph\n    if not na(r_x1)\n        line.new(r_x1, r_y1, bar_index, high[5], color=color.new(color.red, 30), width=2, extend=extend.right)\n    r_x1 := bar_index - 5\n    r_y1 := high[5]\nif pl\n    if not na(s_x1)\n        line.new(s_x1, s_y1, bar_index, low[5], color=color.new(color.green, 30), width=2, extend=extend.right)\n    s_x1 := bar_index - 5\n    s_y1 := low[5]\n""", "Automatically draws trendlines connecting recent pivot highs (resistance) and pivot lows (support).", "trend", "Paste onto your price chart. Red lines connect pivot highs — resistance trendlines. Green lines connect pivot lows — support trendlines. A break through either line is a potential signal. Adjust Lookback to tune sensitivity.", "beta"),
    "avg_daily_range":("Average Daily Range",       lambda: """//@version=6\nindicator("Average Daily Range", overlay=false)\nint len = input.int(14, "Length", minval=1)\nfloat dr  = request.security(syminfo.tickerid, "D", high - low)\nfloat adr = ta.sma(dr, len)\nplot(dr,  "Daily Range", color=color.new(color.blue,  20), linewidth=1)\nplot(adr, "ADR",         color=color.new(color.orange, 0), linewidth=2)\nbgcolor(dr > adr * 1.5 ? color.new(color.red, 90) : na, title="High Range Day")\n""", "Average daily high-to-low range over N days. Shows expected daily move size for position sizing.", "volatility", "Add as a separate panel. The orange ADR line shows the average daily range. When the blue daily range bar exceeds 1.5× ADR (red background), today is an unusually large-range day — adjust stops accordingly.", "beta"),
    "adx":           ("Average Directional Index",  lambda: """//@version=6\nindicator("Average Directional Index", overlay=false)\nint len = input.int(14, "Length", minval=1)\n[diPlus, diMinus, adx] = ta.dmi(len, len)\nplot(adx,     "ADX",   color=color.new(color.orange, 0), linewidth=2)\nplot(diPlus,  "DI+",   color=color.new(color.green,  0), linewidth=1)\nplot(diMinus, "DI-",   color=color.new(color.red,    0), linewidth=1)\nhline(25, "Trending",  color=color.new(color.gray, 40), linestyle=hline.style_dashed)\nhline(50, "Strong",    color=color.new(color.gray, 60), linestyle=hline.style_dashed)\nbgcolor(adx > 25 and diPlus > diMinus ? color.new(color.green, 92) : adx > 25 and diMinus > diPlus ? color.new(color.red, 92) : na)\n""", "Measures trend strength (ADX) plus directional bias (DI+/DI−). ADX above 25 = trending market.", "trend", "Add as a separate panel. ADX above 25 = trend is strong enough to trade. Green background = uptrend (DI+ > DI−), red = downtrend. ADX below 25 = choppy — avoid trend-following strategies. The DI+/DI− crossover is a directional signal.", "beta"),
}
...
""" + _NAV_CSS + """
    ...
    .ind-tag-new  { background: #1a2d1a; color: #3fb950; border: 1px solid #2a4a2a; }
