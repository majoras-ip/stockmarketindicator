...
INDICATORS = {
    ...
    "psar":          ("Parabolic SAR",        lambda: "//@version=6\nindicator(\"Parabolic SAR (200 EMA Filtered)\", overlay=true)\nfloat start = input.float(0.02, \"Start\",     step=0.01)\nfloat inc   = input.float(0.02, \"Increment\", step=0.01)\nfloat maxv  = input.float(0.2,  \"Max\",       step=0.05)\nfloat psar  = ta.sar(start, inc, maxv)\nfloat e200  = ta.ema(close, 200)\nbool  bull  = close > psar\nbool  above = close > e200\n// Only show SAR dots aligned with the 200 EMA trend\nbool  show  = (bull and above) or (not bull and not above)\nplot(e200, \"200 EMA\", color=above ? color.new(color.green, 40) : color.new(color.red, 40), linewidth=1)\nplotshape(show ? psar : na, style=shape.circle, location=location.absolute,\n    color=bull ? color.new(color.green, 0) : color.new(color.red, 0), size=size.tiny)\nbgcolor(above ? color.new(color.green, 96) : color.new(color.red, 96))\n", "Parabolic SAR filtered by the 200 EMA — only shows dots that align with the long-term trend, cutting out counter-trend noise.", "trend", "Paste onto your price chart. Green background = price above 200 EMA (uptrend) — only bullish SAR dots shown. Red background = downtrend — only bearish dots shown. Counter-trend dots are hidden. Use the remaining dots as trend-following entries and trailing stops.", "beta"),
